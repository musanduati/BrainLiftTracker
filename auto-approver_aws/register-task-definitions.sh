...
if [ ! -f "ecs-task-definition-twitter-test.json" ]; then
    echo "❌ Test task definition not found. Generate with: ./aws_complete_setup.sh --generate-tasks"
    exit 1
fi

if [ ! -f "ecs-task-definition-twitter-production.json" ]; then
    echo "❌ Production task definition not found. Generate with: ./aws_complete_setup.sh --generate-tasks"
    exit 1
fi
...
TEST_REVISION=$(aws ecs register-task-definition \
    --cli-input-json file://ecs-task-definition-twitter-test.json \
    --region us-east-1 \
...
PROD_REVISION=$(aws ecs register-task-definition \
    --cli-input-json file://ecs-task-definition-twitter-production.json \
    --region us-east-1 \
    --query 'taskDefinition.revision' \
    --output text)
...
aws ecs list-task-definitions \
    --family-prefix twitter-automation \
    --status ACTIVE \
    --region us-east-1 \
...
echo "🧪 Test with: ./aws_complete_setup.sh --test"
